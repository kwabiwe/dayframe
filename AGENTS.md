...
- General offline event capture goes through the queue in `apps/mobile/src/lib/api.ts`. Health capture first commits its backend/workspace/user-owned source journal in `apps/mobile/src/lib/healthSyncStore.ts`, then hands the immutable event to that queue without changing its ID or payload.
- Mobile connectivity has one process-wide NetInfo owner and one root presentation/VoiceOver owner. Each primary tab places the same fixed 44-point icon slot immediately after the shared Dayframe wordmark, so state changes never move the header. Confirmed offline uses a neutral cloud-slash. Ordinary online durable work waiting or in retry backoff stays in the background and leaves the slot visually empty; only an active reconnect/recovery transmission shows rotating neutral sync arrows. A live non-zero-to-zero transition uses a neutral cloud-check for about two seconds before the slot becomes visually empty. Reduce Motion stops rotation. Permanent timer Stop or time-entry Edit/Delete rejection uses the neutral cloud-X action to open Settings > Sync & diagnostics; confirmed offline remains the higher-priority one-slot state. A permanently rejected Stop must restore server timer truth and expose owned Retry/Discard controls. Never replace durable queues, clear pending work, infer completion from reachability/pass outcome, or disable an offline-capable action solely because reachability reports offline. Project only retryable queued Start/Edit/Delete/Stop work over every cached or fetched Dashboard bootstrap. Validate network transitions, layout and timer convergence on a physical iPhone; Simulator and unit results are not device evidence.
- Already-started durable timer Start/Switch/Stop/Edit/Delete delivery may hold one shared finite iOS background assertion only while immediate timer work is transmitting. Native code owns the UIKit identifier and exact-once expiry cleanup; logout, account replacement, cancellation, settlement and teardown release it without deleting queued work. This is best-effort process time, not guaranteed connectivity or force-quit completion. Review, Location Intelligence and ordinary bootstrap remain foreground-owned.
...
- Mobile `preview` builds must use `https://dayframe-staging.vercel.app` with `com.layereight.dayframe.staging`; production/TestFlight builds must use `https://dayframe-web.vercel.app` with `com.layereight.dayframe`. Their App Groups, Keychain groups, URL schemes, Live Activity extensions, and on-device state are isolated. Verify the signed identities and shared-container hand-off before treating a staging build as device evidence.
