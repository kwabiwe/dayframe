...
- Mobile Dynamic Type remains enabled. Use the scoped roles and caps in `docs/brand-style-guide.md`; leave body, explanations, help, warnings and errors uncapped. A cap never proves that a label fits: measure the real component width and wrap/stack before content or a 44-point action becomes unusable. Keep complete time/count/duration and spoken context; do not globally disable scaling or manually scale twice. Reports keeps its separate approved dense-role contract.
- Reports uses the local dense-role caps in the brand guide: one-line category rows in a zero-gap divider list with a 38–42-point ordinary measured pitch, name-only ellipsis, complete numeric columns and natural spoken durations. Never restore the large-text stacked layout. Native intrinsic widths reserve the current hour capacity, not speculative digits; axis gutters shrink as well as grow. Category order is exact duration descending with stable-key ties, shared by list and donut. The shared popup calendar uses six explicit nowrap rows of seven equal-flex cells, square circular endpoints and continuous range bands; Reports centres the reference picker's 349-point inner cap. Do not substitute percentage wrapping, infer glyph fit from a multiplier cap, or add a Clear/reset-to-Today action.
...
- [ ] Focus is visibly neutral grey—not blue—on every web interactive control, while text, icon, border and chart contrast remain legible in System, Light and Dark modes.
