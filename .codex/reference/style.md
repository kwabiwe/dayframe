...
- Reports uses the local dense-role caps in the brand guide: one-line category rows, name-only ellipsis, complete numeric columns and natural spoken durations. Never restore the large-text stacked layout. Native intrinsic widths reserve the current hour capacity, not speculative digits; axis gutters shrink as well as grow. The shared popup calendar uses six explicit nowrap rows of seven equal-flex cells, square circular endpoints and continuous range bands; its callers retain timestamp ownership. Do not substitute percentage wrapping or infer glyph fit from a multiplier cap alone.
