...
- Reports uses the local dense-role caps in the brand guide: one-line category rows, name-only ellipsis, complete numeric columns and natural spoken durations. Never restore the large-text stacked layout. The shared popup calendar uses 42 fixed slots, square circular endpoints and continuous range bands; its callers retain timestamp ownership.
