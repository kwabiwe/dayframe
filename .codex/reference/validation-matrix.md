...
- On web Calendar, single-click a completed, running, blank/uncategorized, tagged/place-backed, short, overlapping, lower-edge and cross-midnight fragment. Verify exactly one compact editor portal opens from the selected visible fragment; no old bottom card remains; Description/Category/Start and completed Finish are the only editable fields; Duration/Elapsed, compact overlap copy and differing Start/Finish date context are accurate.
- Verify pointer-open on an existing block leaves focus unchanged, while click-create and Space-open focus Description. Enter and double click open the advanced editor, and every icon-only Play/Delete/Close action has an accessible name and at least a 44px target. Tab and Shift+Tab traverse the create editor; keyboard focus is neutral grey on Description/Category/Start/Finish, Save, Go back, Discard, header icon actions and Category options; selected/accent state stays distinct and invalid focus keeps its danger cue. First Escape closes Category and the next reaches the editor. Close/Escape and outside dismissal preserve dirty-draft confirmation; a clean outside click dismisses, while a changed draft blocks the outside action and crossfades the stable footer to `Discard unsaved changes?`; Go back and Escape restore the exact field focus and draft, successful create returns focus to the Calendar grid, Discard closes without saving, and rapid outside/navigation/block clicks cannot leak through the decision.
...
- In Calendar create mode verify blank Description, Uncategorized Category, exact Start/Finish, full `0:30:00` Duration, Close/Save, no Play/Delete and no automatic focus stop on the provisional anchor. Save the untouched baseline and an edited/overlapping/cross-midnight draft; inspect the request and persisted rows for exactly one POST, `tagNames: []`, trimmed optional Description/Category, no place/project/client/hidden fields and exact timestamps. During the pending POST, target outside/anchor clicks, Escape, date/scope/view navigation, scroll-away and anchor loss: the same editor instance, draft and anchor must remain. The outside pointer must still be consumed even though the busy guard blocks dismissal. Rapid Save produces one entry/event, server failure retains the exact editor/draft/anchor and announces the error in its live region, success waits for canonical refresh before closing, and a concurrently running timer remains byte-for-byte unchanged.
- Inspect completed compact-save requests and persisted rows: no-change emits no PATCH; changed saves contain only dirty owned fields; clear values become `null`; untouched timestamp seconds and tag/place/legacy metadata survive; edited times normalize seconds to zero; invalid/future/reversed input remains open with the exact draft; and cross-midnight timestamps keep their original local date components. While an unrelated timer mutation is busy, a completed-entry dirty PATCH must remain enabled and save normally; create/running-entry saves remain gated. Exercise stoppedAt-only API PATCHes against the stored Start, future Finish, Finish-before-Start and valid partial combinations.
