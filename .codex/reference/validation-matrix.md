...
- Focused tests for all/include/none, All mixed semantics, unavailable and same-name IDs, all-context denominators, >8 categories, active replacement, Review exclusion and category/bucket/Total reconciliation. No coverage union or R1 Pie/Bars expectation remains in mobile Reports.
...
- Request/UI tests: Year-to-Month and custom-to-preset races, session/account replacement, timeout/failure, exact-range offline cache, zero selection, no stale relabelling, no filter network request, 44-point targets, scalable category rows and invariant donut geometry at maximum font scale.
- Charts/motion: nice zero-based axis, >60m/hour concurrency, zero fill, sparse labels, one anchored tooltip, rapid replace/outside/range/filter dismissal, stable ID height/arc updates, once-only first focused populated entrance and Reduce Motion. Record complete simulator transitions and physical-iPhone frame pacing; screenshots are insufficient.
