...
- Verify pointer open leaves focus unchanged, Space opens and focuses Description, Enter and double click open the advanced editor, and every icon-only Play/Delete/Close action has an accessible name and at least a 44px target. Keyboard focus is neutral grey on Description/Category/Start/Finish, Save, Go back, Discard, header icon actions and Category options; selected/accent state stays distinct and invalid focus keeps its danger cue. Close/Escape discard and return focus; first Escape closes Category. A clean outside click dismisses, while a changed draft blocks the outside action and crossfades the stable footer to `Discard unsaved changes?`; Go back and Escape restore the exact field focus and draft, Discard closes without saving, and rapid outside/navigation/block clicks cannot leak through the decision. Empty Calendar clicks do not create entries.
