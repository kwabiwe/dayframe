...
- Verify pointer open leaves focus unchanged, Space opens and focuses Description, Enter and double click open the advanced editor, and every icon-only Play/Delete/Close action has an accessible name and at least a 44px target. Close/Escape/outside click discard, Close/Escape return focus, first Escape closes Category, a directly selected visible block replaces the old draft, and empty Calendar click only dismisses.
