...
Mobile Reports Stage A Revision 3 additionally requires:
...
- Range tests: Monday weeks, calendar months/leap years, reversed/same-day custom endpoints, no future/>366 days, custom 1/2/31/32/180/181/366 thresholds and Europe/London 23/25-hour DST days with unique repeated-hour labels.
...
- Request/UI tests: Year-to-Month and custom-to-preset races, session/account replacement, timeout/failure, exact-range offline cache, zero selection, no stale relabelling, no filter network request, 44-point targets, mandatory single-line category rows, name-only ellipsis, complete numeric columns and invariant donut geometry at maximum font scale.
- Shared picker: 42 cells/six reserved rows in Reports and running/stopped/Add Time. Preserve hours/minutes/seconds, maxDate, date-to-time handover, keyboard and Done. Check bare ticks, all presets drafted inside the sheet, no draft fetch, Cancel/backdrop/blur discard, unchanged Done, 366/367-day bounds and future preset highlighting without future custom taps.
- Layout: default/MAX/Bold Text at 320/375/390/430; 100%, <1%, 142:18:06, 9999:59:59 and 100000:00:00 with one-line rows and full spoken names. Verify local caps, no duplicate donut total and no cropped badge/glyphs on the identified native build.
- Charts/motion: fitted non-scrolling plot with one nearest-slot target, selectable zero buckets, adjustable VoiceOver and 44-point Previous/Next rather than overlapping targets; three-tick nice zero-based axis, >60m/hour concurrency, zero fill, sparse labels, one anchored tooltip, rapid replace/outside/range/filter dismissal, stable ID height/arc updates, once-only first focused populated entrance and Reduce Motion. Record complete simulator transitions and physical-iPhone frame pacing; screenshots are insufficient.
