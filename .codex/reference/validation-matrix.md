...
- In mobile Location Evidence, verify the fill-led Where/What/When editor for matched and unmatched stays, and What/When only for a commute. An unmatched stay must not render a large `No saved place` answer card. For stays, retain, clear, and switch nearby saved places; search Apple MapKit, select/clear a stale result, and verify the UI states that a new result will be saved. Exercise unavailable/offline search and map-pin fallback. Choose existing/Uncategorized categories without any Create option, edit activity, adjust same-day/cross-day start/end fields, reject invalid/reversed ranges, and verify unchanged hidden precision survives. Each primary path must emit exactly one atomic action (`edit_and_confirm`, `change_place_and_confirm` with edit, or `save_place_and_confirm` with edit), retain the full draft on failure, and keep split, merge, record-once, and ignore under More options.
- Check normal and Reduce Motion for place-result entrance/replacement/clear, More-options entrance/reflow/exit, date-picker interruption, rapid search/disclosure actions, and failed resolution. Check 44-point targets, VoiceOver selection/value labels, Dynamic Type stacking/scrolling, Light/Dark/System, and no horizontal overflow. This React Native/API change does not require a new native-module implementation build, but staging validation must use a preview binary that already contains the established `dayframe-place-search` module.
...
- On mobile, cover zero-signal foreground replay, local current-time processing, five-by-100 native drain bounds, five-batch upload bounds, permanent rejection continuation, retryable-stop behavior, replay throttling, forced-foreground bypass, concurrent coalescing, account isolation, and `401/403` session clearing. Settings/export diagnostics must remain coordinate-free.
