...
- At phone widths, verify Add Time Suggestions, Tags and Start/Finish date-time panels remain fixed to the visual viewport rather than a query-container ancestor. Start and Finish retain programmatic names, both compact triggers show their calendar date, Finish shows a visible/accessibly described `+N` for cross-midnight windows, and overlap feedback wraps on its own row without clipping.
