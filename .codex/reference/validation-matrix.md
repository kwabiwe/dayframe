...
- Verify the three active V2 rollout modes: `v2_shadow`, `v2_review`, and `v2_enabled`. Prove every V2 mode suppresses competing V1 semantics, shadow is fully silent, a same-mode client acknowledgement is required for semantic cutover, and pre-cutover shadow segments cannot backfill. `v1` is deprecated compatibility coverage only and must never be a default, fallback, or operational rollback. The checked-in/default environment value remains `v2_shadow`.
- In `v2_enabled`, prove one finalised, continuous, route-backed `medium_high`/`high` commute between two saved Dayframe places creates exactly one confirmed category-only `Commute` entry and no Review item. Prove unknown endpoints, endpoint-only evidence, invalid/wide bounds, excessive internal gaps and overlaps greater than five minutes remain Review-first; exercise every condition of the medium saved-route exception. Remove the overlap and replay to prove an existing Review item is not promoted; ignore it and replay to prove the terminal decision remains ignored; delete an automatic commute and replay to prove it is not recreated.
...
- Verify `Total logged`, `Time covered`, overlap markers, and goal-covered semantics across Dashboard, Timeline/History, web Reports, and Settings copy. Mobile Reports Revision 3 shows only summed `Total`; overlapping confirmed activity counts independently, with no coverage metric. Exercise optimistic Stop at non-zero seconds while its aggregate refresh is deferred and assert exact bucket sums rather than rounded labels.

Mobile Reports Stage A Revision 3.2 additionally requires:

- Focused tests for all/include/none, All mixed semantics, unavailable and same-name IDs, selected-only denominators, >8 categories, active replacement, Review exclusion and category/bucket/Total reconciliation. No coverage union or R1 Pie/Bars expectation remains in mobile Reports.
- Range tests: one daily bucket for Today/custom one day; Monday weeks; calendar months/leap years; reversed/same-day custom endpoints; no future/>366 days; custom 1/2/31/32/180/181/366 thresholds; and Europe/London 23/25-hour DST days that remain one bucket with actual boundaries. Freeze 11 September 2026 10:00 Europe/London and retain full Week/Month/Year windows while every future bucket contribution is zero.
- API tests: session first, app-read scope, two users in one workspace plus cross-workspace isolation, strict bounded partition/body validation, one captured instant, no sensitive/raw history payload, DB-side fractional aggregation. Execute synthetic fixtures in disposable localhost PostgreSQL.
- Request/UI tests: Year-to-Month and custom-to-preset races, session/account/lifecycle replacement, timeout/failure, exact-range partition-aware offline cache, zero selection, no stale relabelling, no filter network request, 44-point control targets, mandatory single-line category rows, name-only ellipsis, complete numeric columns and invariant donut geometry at maximum font scale. Verify exact-duration descending order, stable-key ties, running-timer crossover and identical donut/list order through removal/restoration.
- Shared picker: 42 cells/six reserved rows in Reports and running/stopped/Add Time. Preserve hours/minutes/seconds, maxDate, date-to-time handover, keyboard and Done. Reports centres the shared 349-point inner cap (308 points at a 320-point host), uses four equal non-overlapping 44-point preset targets around approximately 34-point fills and has no duplicate title. Check bare ticks, all presets drafted inside the sheet, no draft fetch, handle/backdrop/blur discard, unchanged Done, no visible Cancel or Clear/reset action, 366/367-day bounds and future preset highlighting without future custom taps.
- Layout: default/MAX/Bold Text at 320/375/390/430; zero sibling gap and 38–42-point ordinary category pitch; 100%, <1%, 142:18:06, 9999:59:59 and 100000:00:00 with one-line rows and full spoken names. Verify local caps, no duplicate donut total and no cropped badge/glyphs on the identified native build.
- Charts: one centred daily Today/custom-day bar; seven aligned Week labels with weekday and `DD/MM`; Month/custom first/last date anchors; twelve Year slots with indices 0/2/4/6/8/10 at normal width and deliberate quarterly fallback only when measured fit requires it. Keep all bars within one non-scrolling plot, including future zero buckets, one nearest-slot target, selectable zero values, adjustable VoiceOver and 44-point Previous/Next rather than overlapping targets; retain three nice ticks, zero fill and a bounded tooltip using actual clipped full dates. Prove explicit outside presses and semantic range/filter/focus/background changes dismiss it, while plot/Previous/Next/Close actions, theme/live timer updates and vertical page scrolling cannot trigger or suppress dismissal through a screen-wide touch revision.
- Motion/sheets: both Reports sheets use the app-root iOS full-window overlay and `SwipeDismissSheet` as the only slide/scrim owner, with handle-only pan ownership. The portal hides native tab chrome through the coordinated exit; verify the identified 52-point native-tab control plus the device bottom safe-area reservation are covered exactly once by matching sheet translation, backdrop extension and bottom padding, leaving zero exposed viewport while Done/Apply rests above the safe area. The range body never scrolls: all four presets, range caption, month header, weekdays and six calendar rows remain visible together, with Done outside that fixed body. In filters, Search and Apply remain fixed while only category rows scroll, without an indicator. Exercise entrance, rejected/accepted drag, Done/Apply/backdrop/escape, double action, Apply/swipe race, interrupted entrance, rapid reopen, keyboard-open dismissal, stale presentation IDs, opener focus and current-account/lifecycle invalidation in normal and Reduce Motion. The host remains mounted until one exit callback and produces exactly one commit or discard. Record complete simulator transitions and physical-iPhone frame pacing; source tests/screenshots are insufficient.
- Before merge: exact Ready Preview manually promoted to stable staging, signed com.layereight.dayframe.staging identity, STAGING badge and baked staging API. Run all ranges/filter/custom/tooltips in Light/Dark/System, default/large/MAX Dynamic Type, VoiceOver and Reduce Motion, offline/warm/resume/account replacement. Retain timer Start/Stop/Switch/Edit/Delete/Undo, Today categories/tags, Calendar, safe Review action, Health, Location, Settings and reconnect checks. Record PASS/FAIL/NOT RUN against exact SHA/build; no production data/configuration and no automatic merge.
...
5. Reconnect then immediately disconnect: cloud-slash supersedes other presentation and no stale timer hides it. Force a retryable owner failure and verify the header becomes visually empty while queued work is retained/backing off, shows rotating arrows only during the next live reconnect/recovery attempt, and exposes no raw/internal API alert. Force a permanent time-entry rejection and verify the cloud-X action opens Settings > Sync & diagnostics; while confirmed offline, cloud-slash remains the one-slot priority and cloud-X returns after reconnect.
...
38. Timer start/stop regression: while offline perform Start → Edit description/category → Stop, pull-to-refresh repeatedly, force-quit and relaunch, then reconnect. One final stopped entry remains continuously visible and converges exactly once on web. Repeat Start-only and concurrent-drain cases. Seed older Health, Location Evidence, and Location Intelligence events ahead of a newer explicit Start/Switch: the explicit timer may use the finite timer phase first, but every older event must remain durable, process once on foreground, preserve its original `occurredAt`, and never replace the newer timer; eligible overlapping Health may coexist without changing the active timer. While recovery's final bootstrap is in flight, tap Stop and separately save an Edit; the recovered snapshot must be rejected through the ordinary mutation-revision/deletion guard, queue a fresh projected load and never visibly resume or revert the entry. For online Start, Switch, Stop, Edit and Delete, background immediately after the action and verify acknowledgement/settlement when iOS grants time; on native expiry or offline failure verify exact-once task cleanup, no loop, retained work and one retry after foreground/relaunch. Repeat logout and account replacement during transmission.
...
- `scripts/seed-review-performance-fixtures.ts` creates/removes an owned temporary JSON bundle with explicit synthetic workspace/user UUIDs, five Review types, overlapping/non-overlapping windows, historical peers and 8/400-sample evidence DTOs. It accepts no database/API configuration or real account identity. This is a local fixture bundle, not a staging database seeder; signed-device staging fixture ingestion and timing evidence remain a separate acceptance step.
...
## Health capture durability

Run the Health capture lifecycle and real SQLite suites for atomic checkpoint rollback, same-ID interrupted handoff, owner/backend isolation, split-query episode reconstruction and deletion preservation. Run `npx tsx scripts/validate-health-source-decisions.ts` only against a disposable local `_test` database with the resolution-link migration. A native build is required for Expo Crypto. Physical HealthKit and staging server/mobile visibility remain separate release checks; an empty queue is not evidence of source visibility.
