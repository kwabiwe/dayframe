...
- Mobile explicit Stop posts to `/api/events` with its original `occurredAt`, stable `clientEventId`, `rawPayload.stopScope=entry`, and exact canonical `targetEntryId`. Success, duplicate, and `superseded` acknowledge the durable intent; `timer_busy` is a stable retryable `503`.
