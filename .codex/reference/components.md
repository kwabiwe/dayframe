...
- Compact Calendar Description, Category, Start and Finish controls use the neutral control-border focus perimeter rather than the global blue semantic focus token. A clean outside click dismisses; an outside click after any exact draft-value change must block the underlying action and replace the stable footer in place with `Discard unsaved changes?`, `Go back`, and destructive `Discard`. Go back or Escape returns focus to the field the user was editing without clearing the draft; Discard uses the editor's existing exit owner.
- Compact Calendar saves use dirty-field partial PATCH payloads. Untouched timestamps remain byte-for-byte canonical, edited times normalize to minute precision on the original timestamp's local date, and cross-midnight Start/Finish keep their separate original dates. A no-op Save closes without a request; failed saves retain the exact draft and error.
...
- In editable web Description controls, a selected tag is a plain-text remove action with no default fill and a thin visible `X`. Hover/focus may add only the subtle accent fill and darker accent text, with the same compact `6px` radius as Calendar blocks; clicking the text or `X` removes the tag. Keep this shared between the persistent timer and Edit time block instead of styling each surface separately.
