...
- Manual-entry Suggestions must reuse timer suggestion ranking and metadata without invoking timer start. Selecting one fills the manual draft's Description, Category, and Tags. Cap the visible result set to a documented number of complete fixed-height rows; never crop a partial final row. Its Tags/Suggestions surfaces remain out of flow, anchor to their actual triggers, and must not require scrolling the dialog merely to reveal the options.
- Add time must reuse the timer Category option treatment rather than a separate native select. Date/time controls retain native click-to-select behaviour but use the shared Dayframe themed surface, typography, radius and focus treatment. Dialog child surfaces must preserve the dialog's rounded silhouette without forcing floating pickers back into a clipping container.
