...
- Compact Calendar Description, Category, Start and Finish controls use that neutral focus treatment. A clean outside click dismisses; an outside click after any exact draft-value change must block the underlying action and switch the fixed-geometry footer region to `Discard unsaved changes?`, `Go back`, and destructive `Discard`. The same reserved region owns normal actions, validation/server errors and overlap information with priority `discard > error > overlap > normal`; state changes must not resize or move the editor. Go back or Escape returns focus to the field the user was editing without clearing the draft; Discard uses the editor's existing exit owner.
...
- [ ] Calendar compact editing preserves pointer/Space/Enter/double-click focus semantics, discard/replacement rules, partial-payload timestamp fidelity, active-timer gate ownership, anchored viewport geometry, failure rollback, and Reduce Motion.
