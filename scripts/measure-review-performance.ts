...
import { REVIEW_PERFORMANCE_PROFILES, SYNTHETIC_REVIEW_NOW, syntheticReviewBootstrap } from "./fixtures/review-performance";
...
try {
  db.exec('PRAGMA journal_mode=WAL; CREATE TABLE synthetic_cache (id TEXT PRIMARY KEY, snapshot TEXT NOT NULL);');
  ...
  const measurements=REVIEW_PERFORMANCE_PROFILES.map(count=>{
    ...
    db.exec('DELETE FROM synthetic_cache; BEGIN IMMEDIATE;');
    ...
    return {reviewItems:count,uniquePeerEntries:250,payloadBytes:Buffer.byteLength(JSON.stringify(data.reviewItems)),legacy,prepared,cacheHydration};
  });
