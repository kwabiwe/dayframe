...
export function CategoryPicker({
  ...
  useEffect(() => {
    ...
    function closeOnOutside(event: MouseEvent) {
      if (!rootRef.current?.contains(event.target as Node) && !isCreateBusy) closePicker(false);
    }
  ...
  return (
    <div
      ...
      onBlur={(event) => {
        if (open && !isCreateBusy && !event.currentTarget.contains(event.relatedTarget as Node | null)) {
          closePicker(false);
        }
      }}
      ref={rootRef}
