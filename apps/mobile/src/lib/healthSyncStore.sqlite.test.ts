  ...
  return [
    {
      ...
      event: {
        ...
        description: "Sleep",
        rawPayload: { startedAt, stoppedAt, samples: samples.map((sample) => sample.value) }
      }
    }
  ];
}
async function page(
  ...
  const checkpoint = await store.getHealthCheckpoint(active, "sleep", options.repair);
...
describe("Health journal real SQLite boundaries", () => {
  it("reconstructs a complete revision across deltas while preserving the original payload and ID", async () => {
    ...
    expect(String(deliveries[1].client_event_id)).toMatch(/:[0-9a-f]{64}:[0-9a-f]{64}$/);
  });
  ...
  it("advances its durable anchor offline without claiming server acknowledgement", async () => {
    ...
    expect((await store.getHealthCheckpoint(owner, "sleep")).anchor).toBe("anchor-1");
    expect(
      db.prepare("select state,acknowledgement_json from health_deliveries").get()
  ...
  it("repeats the exact handoff ID after a crash between stores", async () => {
    ...
    const enqueue = vi.fn(async (event: Parameters<typeof import("./api").enqueueEvent>[0]) => {
      queue.set(event.localId!, event);
      return [];
    });
    mocks.failHandoff = true;
    await expect(store.handoffHealthEvents(owner, enqueue, () => true)).rejects.toThrow(
      "crash after"
    );
    vi.resetModules();
    store = await import("./healthSyncStore");
    ...
    expect(db.prepare("select state from health_deliveries").get()!.state).toBe("queued");
  });
  it("rolls back late query results after owner replacement", async () => {
    let checks = 0;
    await expect(page([sample("early")], { isCurrent: () => ++checks === 1 })).rejects.toThrow(
      "owner changed"
    );
    expect(count("health_samples")).toBe(0);
    expect(count("health_checkpoints")).toBe(0);
  ...
  it("isolates cloned identical account IDs across stable backends", async () => {
    ...
    expect((await store.getHealthCheckpoint(production, "sleep")).anchor).toBeNull();
    await page([sample("early")], { owner: production });
    expect(count("health_checkpoints")).toBe(2);
    const ids = db.prepare("select distinct client_event_id from health_deliveries").all();
  ...
  it("records deletion as unresolved source correction without shrinking or replacing delivered intent", async () => {
    ...
    expect(db.prepare("select client_event_id,payload_json from health_deliveries").get()).toEqual(
      original
    );
    await page([late]);
    expect(
      db.prepare("select deleted,sample_json from health_samples where sample_id='late'").get()
  ...
  it("keeps repair progress independent of the normal delta contract", async () => {
    ...
    expect((await store.getHealthCheckpoint(owner, "sleep", repair)).anchor).toBe("anchor-2");
  });
  it("resumes the identical repair window after process restart and keeps its per-type anchor", async () => {
    ...
    expect((await store.getHealthCheckpoint(owner, "sleep", repair)).anchor).toBe("anchor-1");
    expect((await store.getHealthCheckpoint(owner, "workout", repair)).anchor).toBeNull();
  ...
  it("rejects the wrong acknowledgement and persists canonical disposition separately from capture", async () => {
    await page([sample("early")]);
    const id = String(
      db.prepare("select client_event_id from health_deliveries").get()!.client_event_id
    );
    await expect(
    ...
    expect(
      JSON.parse(
        String(
          db.prepare("select acknowledgement_json from health_deliveries").get()!
            .acknowledgement_json
        )
      )
    ).toMatchObject({ eventId: "event", timeEntryId: "entry" });
    expect(db.prepare("select state from health_deliveries").get()!.state).toBe("acknowledged");
  });
  it("retains unacknowledged source and payload beyond retention then expires only acknowledged raw data", async () => {
    ...
    expect(db.prepare("select sample_json from health_samples").get()!.sample_json).not.toBeNull();
    expect(
      db.prepare("select payload_json from health_deliveries").get()!.payload_json
    ).not.toBeNull();
    const id = String(
      db.prepare("select client_event_id from health_deliveries").get()!.client_event_id
    );
    ...
    await store.pruneAcknowledgedHealthCapture(owner, later);
    expect(db.prepare("select sample_json from health_samples").get()!.sample_json).toBeNull();
    expect(
      db.prepare("select payload_json,acknowledgement_json from health_deliveries").get()
    ).toMatchObject({ payload_json: null, acknowledgement_json: expect.any(String) });
  });
  it("keeps source corrections and recorded time through explicit keep and later replay", async () => {
    ...
    expect(await store.keepRecordedHealthTime(owner, correction.correctionId, () => true)).toBe(
      true
    );
    await page([early]);
    expect(count("health_deliveries")).toBe(1);
