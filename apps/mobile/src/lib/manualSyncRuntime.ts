...
import { cacheDashboardBootstrap, synchroniseReviewMutations } from "./reviewSyncStore";
...
const deliveries = createOwnerSyncCoalescer<ManualLaneResult>();
...
export async function synchroniseDeviceNow(options: { date?: string } = {}) {
  ...
  const deliver = (context: ManualSyncContext) =>
    deliveries.run(
      key,
      true,
      async () => {
        if (context.signal.aborted || !(await isCurrent())) return { outcome: "cancelled" };
        const result = await syncQueue({ forceRetry: true, signal: context.signal });
  ...
  const capture =
    ...
    async (context: ManualSyncContext): Promise<ManualLaneResult> => {
      ...
      const selected =
        type === "sleep"
          ? preferences.sleep
          : Object.entries(preferences).some(([kind, value]) => kind !== "sleep" && value);
      if (!enabled || !selected) return { outcome: "complete", stage: "disabled" };
      if (!(await isCurrent()) || context.signal.aborted) return { outcome: "cancelled" };
      const result = await (type === "sleep" ? importHealthKitSleep : importHealthKitWorkouts)({
        signal: context.signal
      });
      return { outcome: "complete", stage: result.importedCount ? "captured" : "query_completed" };
    };
  ...
  const pass = runManualSync(
    {
      ...
      async location(context) {
        if (!mobileAccountOwnersEqual(owner, await getActiveLocationAccountIdentity()))
        ...
        if (result.reason === "v1") return { outcome: "complete", stage: "disabled" };
        return {
          outcome: result.outcome ?? "partial",
          changed:
            (result.acknowledgedCount ?? 0) > 0 ||
            ("semanticSegmentCount" in result && (result.semanticSegmentCount ?? 0) > 0),
      ...
      async refresh(context) {
        ...
        try {
          ...
          const projected = projectDurableLocalWork(bootstrap, await readDurableLocalWork(owner));
      ...
      classifyError(error) {
        if (error instanceof AuthRequiredError) return "authentication_required";
        if (
