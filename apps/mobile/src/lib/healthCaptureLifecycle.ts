...
export function boundedHealthQuery<T>(
  ...
  return new Promise((resolve, reject) => {
    ...
    const abort = () =>
      finish(() =>
        reject(Object.assign(new Error("Health capture cancelled."), { name: "AbortError" }))
      );
...
export function createHealthCaptureCoalescer() {
  const active = new Map<string, { promise: Promise<unknown>; dirty: boolean }>();
  return {
    run<T>(key: string, capture: () => Promise<T>, markDirty = false): Promise<T> {
      const current = active.get(key);
      if (current) {
        if (markDirty) current.dirty = true;
        return current.promise as Promise<T>;
      }
      const entry = { promise: Promise.resolve<unknown>(undefined), dirty: false };
      entry.promise = (async () => {
