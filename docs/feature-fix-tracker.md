...
- Branch `codex/pr1-offline-first-review` is based on `origin/main` at `e849d92` and implements cache-first Review/Location Evidence plus local-commit removal and bounded server work. It is implementation evidence only: no Preview, staging database check, TestFlight build, or physical-iPhone result is recorded yet.
...
| Offline Review terminal actions | Implementation pending | Main retains the shipped account-scoped Review outbox baseline. Branch `codex/pr1-offline-first-review` upgrades its SQLite schema to v4, makes Review cache-first, hides Confirm/Dismiss/complete Edit-and-confirm after local commit across list/detail, restores only canonical-open permanent conflicts at stored anchors, caches validated account-owned Location Evidence for at most seven days/25 items/5 MiB, bounds mobile/server waits, and keeps complex place/split/merge/record actions direct-only. Repository, staging, and physical-iPhone evidence must be completed before merge/release. |
