...
| Mobile explicit Stop durability and contention (Trello #147) | Implementation pending | This branch adds an account-owned storage-only Stop outbox, persist-before-dismiss acceptance, exact `/api/events` replay, 8-second mobile delivery, entry-scoped server locking with 2-second row-lock and 5-second statement deadlines, `timer_busy`, safe timings, legacy unscoped fail-closed handling, and relaunch projection before timer/ActivityKit presentation. Focused tests cover storage, dependency correlation, account isolation, HTTP classification, stale bootstrap projection, route mapping, idempotency, and lock behavior, and a clean unsigned arm64 iOS Simulator build passes; the gated two-connection Postgres test, staging schema/index verification, and physical-iPhone force-quit/offline/contention matrix still require recorded results. |
