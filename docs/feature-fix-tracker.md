...
Last repository audit: 2026-08-21 against `origin/main` at merge `2b732d2` (PR #183).

## Evidence snapshot

- Main includes PRs #149–#183. Since build 93 it has added mobile lifecycle/Keychain recovery and locked-screen Live Activity hardening (#176), web entry-form and Review-evidence follow-ups (#177–#178), mobile session/login/Review-sync ownership guards (#179–#180), documentation/process guardrails (#181), durable account-scoped timer Stop plus Live Activity convergence (#182), and cache-first Review/Location Evidence with local terminal-action acknowledgement (#183).
...
- PR #183 is merged at `2b732d2` with repository coverage for cache-first Review/Location Evidence, local terminal-action acknowledgement, cancellation and bounded server work. It is not included in the repository-recorded TestFlight build 97; no exact PR #183 Preview, signed staging build, or physical-iPhone result is recorded here.
...
| Offline Review terminal actions | Release pending | PR #183 is on main at `2b732d2`. SQLite v4 makes Review cache-first; Confirm, Dismiss, and complete Edit-and-confirm hide after the local transaction; canonical-open permanent conflicts restore at stored anchors; validated account-owned Location Evidence is bounded to seven days/25 items/5 MiB; and complex place/split/merge/record actions remain direct-only. Repository coverage exists, but TestFlight build 97 predates the merge and no exact signed/physical PR #183 evidence is recorded. |
| Global mobile connectivity status and ordered reconnect | Implementation pending | Branch `codex/pr184-global-connectivity-status` adds one NetInfo monitor, passive offline/restored notices, current-HTTP evidence with stale-response protection, and a serialized reconnect epoch that prompts the existing timer Stop, activity-event, Review, Location Intelligence, and bootstrap owners in dependency order. It adds no server endpoint or migration and never replaces durable queues or gates offline-capable actions. Preview, signed staging build, and physical-iPhone transition/recovery evidence remain required before merge/release. |
