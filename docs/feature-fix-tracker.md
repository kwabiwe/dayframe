...
Last repository audit: 2026-08-12 against `origin/main` at merge `fb80074` (PR #169).

## Evidence snapshot

- Main includes PRs #149–#169, including Health sleep reconciliation, offline/mobile editor follow-ups, category-palette grouping, native Calendar creation, unified web editing, Live Activity sync, Timeline quick editing, documentation alignment, consistent contextual web category creation, time-driven Location V2 finalisation replay, and simplified mobile Review/Location Evidence presentation.
...
- Main has moved beyond that mobile binary: PRs #163, #164, #168, and #169 contain later mobile sheet/tag/layout, Location replay, or Review presentation changes. Do not claim those changes are in TestFlight until a newer verified build is recorded.
...
| Mobile Review and Location Evidence presentation | Release pending | Review uses separate activity cards, prominent activity/time, an accessible five-step confidence indicator, compact overlap copy, and user-facing About text without Health diagnostics. Location Evidence leads with activity, minute-level time, and map, then separates Where/What/When correction. Stays can choose nearby saved places or explicitly search Apple MapKit for a new place to save; activity, existing category, and start/end edits commit with the place decision through one atomic action. Split, merge, map-pin fallback, record-once, confirm, and ignore remain available under compact More options. PR #169's exact Preview was promoted to staging and a signed Staging build from `71edfaa` was installed and launched on the attached iPhone; the newer correction editor still needs its own staging/device evidence and production/TestFlight remains pending. |
...
| --- | --- | --- |
| Production Location V2 mode | Code supports `v1`, `v2_shadow`, `v2_review`, and narrow `v2_enabled`; repository defaults intentionally fail closed. | Keep shadow, enable Review-only, or approve the documented high-confidence automatic policy after device/staging evidence. |
