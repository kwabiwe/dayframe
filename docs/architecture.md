...
Mobile Today uses the read-only `POST /api/review/presentation` contract rather than widening bootstrap. It resolves an app-read session and both workspace/user predicates, validates a bounded time-zone/window/page/lookup request, and returns only whitelisted Review, legacy-entry, completed-today, count, lineage and snapshot evidence with private/no-store headers. The service uses a bounded repeatable-read transaction and never takes a timer or Review mutation lock. Today reads its bounded window; the existing Review inbox reads serial keyset backlog pages and exposes an explicit next-page action rather than treating the capped bootstrap list as complete. Cursor/snapshot disagreement is a typed read conflict; a foreground client restarts one generation at most and retains its previous verified cache when a read cannot complete.
...
- `apps/mobile/src/lib/reviewSyncStore.ts`: account-scoped downloaded Review state, resolving/structural Review outbox, v7 bounded whitelisted Review-presentation contexts/terminal evidence, and a validated private Location Evidence presentation cache capped at seven days, 25 items, and 5 MiB;
...
Review SQLite v6 preserves immutable envelopes and optimistic effects while recording contention, reconciliation and validated acknowledgements. V7 adds only owner/backend-bound, strictly parsed display-context/terminal evidence; it neither replaces the outbox nor creates a second canonical entry store. Bootstrap omissions are partial evidence, never a deletion or acknowledgement-pruning proof. Terminal source evidence is read only for the matching account and backend: `missing_review` is a typed lookup result that may persist terminal missing evidence, not a Review-record status. Only a strictly later server `capturedAt` from an explicit same-backend `open` Review record clears it, so delayed or equal-age reads cannot resurrect a terminal source. An acknowledged intent remains until every affected source has explicit terminal evidence and each linked result is explicitly proven completed or missing by scoped presentation evidence, including results outside the cached Dashboard/history window. A legacy needs-review entry is not completed-result proof. The foreground reader selects deterministic whole-mutation batches within the presentation ID bounds, so an older unmaterialised result does not mask newer acknowledgements; the existing store retires each independently proven mutation. When an equivalent accept/confirm receipt has no entry ID, the foreground display reader first resolves all affected sources, follows only its explicit scoped source-to-entry links, then reads those result IDs; receipt or row absence alone cannot retire the intent. Today may render saved-local state from that evidence, but only a current canonical entry contributes completed time. Old envelopes and request hashes remain valid. Owner-scoped ordinary triggers join active delivery, while an explicit manual trigger persists one follow-up. Full sync JSON deadlines include parsing, validation and captured-session checks. Root recovery schedules timer, activity, Review and Location by their own due times; server contention does not imply transport failure. Settings' 45-second manual coordinator settles enabled Health types independently and explicitly awaits event delivery, bounded Health processing, Review, native Location drain/upload/replay and coalesced projected refresh. No local query, empty queue or successful replay proves visibility of a particular source record.
...
- A complete successful mobile account-journal replay atomically replaces its derived segment snapshots with engine state; failed/partial processing cannot prune them. Durable evidence, uploads and Review ownership remain separate. Server replay reconciles obsolete open proposals and holds changed-ID replacements that reuse a protected decision's exact source evidence and occupied interval. Terminal/manual segment fields, canonical entries, receipts and lineage remain protected; no new identity namespace or durable owner is introduced.
- Saved-place arrival corroboration and its narrow arrival-conflict witness are replay-local analysis in the shared engine. The accepted finite Visit, actual strong observations and callbacks retain their existing evidence IDs; inferred bounds and the ephemeral boundary marker flow through the existing segment/commute persistence and policy path without a schema or public-wire addition. Mobile complete-journal rebuild and server replay therefore produce the same corrected or fallback output while existing journal, upload, Review, lineage, receipt and terminal/manual protection owners remain unchanged.
