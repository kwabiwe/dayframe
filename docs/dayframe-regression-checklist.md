...
- Web Calendar blocks use one primary `Description · Category · #FirstTag +N` line and one secondary full-entry `Duration (Start – Finish)` line. At 40px both appear, at 24–39px the full primary appears, at 18–23px/title-density lanes only Description/Category fallback appears, and smaller/none-density lanes show no text.
