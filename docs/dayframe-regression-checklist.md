...
- A single click/tap or Space on a web Calendar block opens one portalled compact editor anchored to that exact rendered day fragment. Pointer opening does not move focus; Space focuses Description; Enter and double click retain the advanced editor. Its Description/Category/Start/Finish focus perimeter is neutral grey. Close/Escape discard and restore block focus; a clean outside click dismisses, while a changed draft blocks the underlying click and shows `Discard unsaved changes?` in the stable footer. Go back/Escape restore the exact edited field and draft, and Discard closes without saving. Empty Calendar clicks do not create entries.
...
- In the persistent web timer and Edit time block, each selected tag is plain text with no default fill and a thin `X` remove affordance. Hover/focus uses darker accent text, a subtle accent fill and a `6px` rectangle radius; clicking either the tag text or `X` removes it.
