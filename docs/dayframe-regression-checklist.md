...
- A single click/tap or Space on a web Calendar block opens one portalled compact editor anchored to that exact rendered day fragment. Pointer opening does not move focus; Space focuses Description; Enter and double click retain the advanced editor. Close/Escape/outside click discard the local draft, Escape/Close restore block focus, a different visible block replaces the draft directly, and empty Calendar clicks do not create entries.
