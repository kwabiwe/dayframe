...
- A single click/tap or Space on a web Calendar block opens one portalled compact editor anchored to that exact rendered day fragment. Pointer opening does not move focus; Space and click-create focus Description; Enter and double click retain the advanced editor. Completed/new cards always show Start and Finish date/time plus editable `HH:MM:SS` Duration; running cards show editable Start, `Running` Finish and live read-only Duration, then expose Finish/Duration after stop. Start preserves Duration, Finish recalculates it, and Duration moves Finish without feedback loops. Text/date/time fields use a neutral border for pointer or keyboard focus; buttons/links/tabs/dropdown/icon actions use neutral `:focus-visible` only. Close/Escape and outside dismissal preserve the exact `Discard changes?` / `Go back` / `Discard` decision, and the fixed feedback plane keeps `discard > error > overlap > empty` priority without movement.
...
- Native Calendar empty-time creation has one retained UIKit long-press recognizer in the existing scroll coordinator: one finger, a deliberate 0.50-second hold, 11-point movement allowance, and one event only at `.began`. Vertical scroll, horizontal day swipe, pinch, pull-to-refresh, a second finger, day/layout invalidation, the hour axis, and every entry-owned semantic hit frame must cancel or reject it.
- Native Calendar long press floors the selected day's local wall time to the containing 15-minute slot, clamps Start to 23:45, and asks React for an exact 30-minute Add-time draft. Swift emits only `dayKey` and `startMinute`; React owns DST-safe timestamp construction, the existing Add sheet, `createManualTimeEntry`, canonical refresh, errors, and persistence.
- Long press inside an isolated short entry's 44-point target, an overlapping lane's semantic target, or the preceding entry's one-point visual gap must not create. Empty horizontal space beside an occupied overlap lane remains eligible.
