...
- The current review follow-up's static tokens and contracts resolve focus to neutral `rgb(100, 113, 138)` in Dark and `rgb(125, 135, 151)` in Light across fields and actions. Those exact current-head values were not re-measured in a renderer because no browser-control instance was available in the session.
...
- NOT RUN: rendered Light/Dark review-head matrix at 1440x900, 1024x768, 390x844 and 320x700. The required browser-control surface exposed no connected browser instance. Existing untracked QA artifacts were preserved and were not presented as current-head evidence.
