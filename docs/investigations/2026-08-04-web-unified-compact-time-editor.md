...
- Selected tag visuals centre their label inside the 24px wrapper and optically lower only the Lucide `X` by 1px; the existing 44px target and hidden width measure remain shared.
...
- Density follow-up: every readable block and `title` overlap lane uses the same structured first line, independent of the secondary line. Description is primary semibold, Category muted normal, the first tag muted bold and `+N` muted medium. Actual rendered widths at or below 111px retain Description only; below 18px or at `none` density there is no text. The independent secondary line starts at the safe 48px height, which fits both lines with the existing padding. Direct resize remains 48px and above.
