...
profile workspace switching, and sidebar appearance access.
...
- Focused web typecheck passed. The focused component suites passed 18 tests.
- Full lint, workspace typechecks, brand contract, optimized production build,
  and `git diff --check` passed. The full suite passed 775 tests.
- Optimized-build browser QA passed eight evidence groups across Light, Dark,
  and System at desktop, phone, and compact-height layouts. It exercised timer
  pointer Save and Enter as separate single-update paths, persistence after
  reload, no navigation/Loading flash, picker tab order, Appearance contrast,
  compact workspace controls, profile workspace rows, category colour/action
  contracts, the sidebar theme toggle, overflow, and console/runtime errors.
