...
Remaining before this branch is ready: server source-decision guards for prior ignored/deleted Health records; explicit repair/source-correction Settings actions and fresh support diagnostics; the separate environment/session/native migration prerequisite; native rebuild; staging Preview and hands-on device validation. The current backend identity helper is only a journal namespace and does not prove an old global session belongs to that backend. The observed source/import defect is not an attestation of the historical missing night.
