...
Independent review #188: missing `activity_events.resolved_time_entry_id` now returns a typed 503 `database_not_ready` / `required_migration_missing` with the exact hosted/local migration hint, without raw SQL or parameters. Event, Review delivery/reconciliation and Health reprocess routes share that mapping. The existing real transaction, Health logical-unit and Health ingestion validators are mandatory steps in both disposable PostGIS schema jobs. No migration or production operation was performed for this revision.

The ingestion deadline validator accepts either the database deadline or the JavaScript deadline when both expose `operation_timeout` at the effect phase. PostgreSQL 17 can win this race; the test still requires bounded settlement, complete rollback, released ownership and same-ID/concurrent idempotency. The real transaction, Health logical-unit and ingestion validators all pass on the isolated PostgreSQL 17/PostGIS test database.
