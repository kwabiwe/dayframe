...
Stop now completes the bounded direct idempotent event request before ending local ActivityKit state. Successful direct delivery removes the already-durable queue item; timeout/offline failure dismisses locally and leaves replay intact. The direct request remains bounded to 8 seconds (10 seconds total resource time). A fixed, non-sensitive marker in the App Group records only `started`, `context-unavailable`, request/response class, transport failure, or `delivered`, allowing `devicectl` diagnosis after a locked-device test without exposing the session, token, request body, account, or coordinates.
