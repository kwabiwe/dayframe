...
- The native Shortcut catalog/event payload now includes non-secret user/workspace IDs so pending App Intent/Live Activity hand-off events are counted and drained only by their owner. App Intents fail closed while signed out, logout removes the outgoing owner's pending native events plus any unscoped legacy residue, and pre-upgrade unscoped events can therefore be adopted only inside an authenticated account boundary.
